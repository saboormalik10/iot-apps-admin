...
/**
 * The instant a local wall-clock hour begins on a local calendar day.
 *
 * Same two-pass resolution as `localDayBounds`, at `hour` rather than midnight,
 * so a 9am rain day starts at 9am local on a DST changeover day too — adding nine
 * hours to local midnight would land on 8am or 10am there.
 */
export function localHourStart(dayKey: string, hour: number, timeZone: string): number {
  const [y, m, d] = dayKey.split('-').map(Number);
  const guessOffset = offsetMinutesAt(Date.UTC(y, m - 1, d, 12, 0, 0), timeZone);
  const firstPass = Date.UTC(y, m - 1, d, hour, 0, 0) - guessOffset * 60_000;
  const actualOffset = offsetMinutesAt(firstPass, timeZone);
  return Date.UTC(y, m - 1, d, hour, 0, 0) - actualOffset * 60_000;
}

/** The calendar day before `dayKey`, as `YYYY-MM-DD`. */
...
/**
 * When the "day" containing `tsMs` began, for a day that starts at `startHour`
 * local — midnight for a calendar day, 9 for the Bureau of Meteorology's 9am-to-9am
 * rain day. Returns the start instant and the day it is named after (the date on
 * which it began).
 */
export function dayStartAt(tsMs: number, startHour: number, timeZone: string): { startMs: number; dayKey: string } {
  const today = localDayKey(tsMs, timeZone);
  const start = localHourStart(today, startHour, timeZone);
  if (start <= tsMs) return { startMs: start, dayKey: today };
  const yesterday = previousDayKey(today);
  return { startMs: localHourStart(yesterday, startHour, timeZone), dayKey: yesterday };
}
