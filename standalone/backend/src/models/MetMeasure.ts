...
export interface IMetMeasure extends Document {
   ...
   *
   * Absent on per-reading rows written before that (the cloud's history). They are
   * left exactly as they are; nothing needs to migrate, and a
  ...
  source: 'sftp' | 'mobile' | 'stream' | null;
  /**
...
const metMeasureSchema = new Schema<IMetMeasure>(
  {
     ...
     *
     * `source` KEEPS its default, so every row carries its origin.
     */
    source: { type: String, enum: ['sftp', 'mobile', 'stream', null], default: null },
...
// NO expiry. The client keeps weather data indefinitely on the site PC (22 Sep
// 2026) — see PLAN.md, "Data retention". The cloud copy carried a partial TTL
// here (`sftp_ttl_createdAt`, 730 days); first-run setup syncs indexes, which
// drops it from a database an earlier build created.
