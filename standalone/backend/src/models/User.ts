...
export interface IUser extends Document {
  ...
  /**
   * Set when an ADMIN set this password (Users → Reset password): the admin knows
   * it, so the portal makes the user choose their own before anything else.
   * Cleared when the user changes it.
   */
  mustChangePassword: boolean;
  /**
   * Access tokens minted before this moment are refused (see JwtAuthGuard).
   *
   * An access token lives 15 minutes and carries its grants, so revoking a
   * refresh token alone left a reset, suspended or removed account working for up
   * to that long — the very window an administrator resets a password to close.
   */
  sessionsValidFrom: Date | null;
  /**
   * Signed themselves up (STANDALONE_SELF_SIGNUP) and not yet approved. Such an
   * account is also inactive; approving it (activating) clears this.
   */
