...
export class AlertRulesController {
  ...
  async create(@Body() body: CreateAlertRuleDto, @CurrentUser() user: JWTPayload, @ClientIp() ipAddress?: string | null) {
    const rule = await this.service.create(user.organizationId, body, { userId: user.userId, email: user.email ?? '', ipAddress });
    return { data: rule };
  }
  ...
  @UseGuards(JwtAuthGuard, PermissionsGuard)
  @RequirePermissions('alert:write')
  async update(@Param('id') id: string, @Body() body: UpdateAlertRuleDto, @CurrentUser() user: JWTPayload, @ClientIp() ipAddress?: string | null) {
    const rule = await this.service.update(user.organizationId, id, body, { userId: user.userId, email: user.email ?? '', ipAddress });
    return { data: rule };
  }
  ...
  @UseGuards(JwtAuthGuard, PermissionsGuard)
  @RequirePermissions('alert:write')
  async remove(@Param('id') id: string, @CurrentUser() user: JWTPayload, @ClientIp() ipAddress?: string | null): Promise<void> {
    await this.service.remove(user.organizationId, id, { userId: user.userId, email: user.email ?? '', ipAddress });
  }
}
