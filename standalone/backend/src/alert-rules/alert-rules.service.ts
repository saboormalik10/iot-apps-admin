...
type Actor = { userId: string; email: string; ipAddress?: string | null };
...
export class AlertRulesService {
  ...
  async timeline(
    ...
    const field = MET_SENSOR_MAP[rule.sensor];
    ...
    // A sensor with no stored field has no history to reconstruct. Say so rather
    // than returning an empty chart that reads as "the station was silent".
    if (!field) {
