...
import { isEmailConfigured, sendAlertEmail } from '../utils/mailer';
...
export class AlertEvaluationService {
    ...
    for (const rule of rules) {
      ...
      await this.notifications.notify(
        ...
        rule.notifyUserIds.map((id) => id.toString()),
        {
          ...
          title: rule.name,
          /**
           * Written for the person who reads it on the wall screen, not for us.
           *
           * It used to print the rule's machine keys — "wind_speed gt 1m/s —
           * read 18.29m/s" — and, worse, said nothing about WHICH reading 18.29
           * was. A rule fires on the highest (or lowest) reading in the minute,
           * deliberately, so that a gust cannot slip between two averages; but
           * the dashboard shows that minute's AVERAGE, so the two numbers
           * legitimately differ and the alert looked wrong. It now says so.
           *
           ...
           */
          body:
            `${sensorLabel(rule.sensor)} ${conditionPhrase(rule.condition)} ${rule.threshold}${rule.unit}` +
            ` — ${extremeWord(rule.condition)} ${displayValue}${rule.unit} in that minute`,
          data: {
  ...
  private async emailRecipients(
    ...
    // Off unless an email server is configured — the web alert has already gone
    // out by the time this runs, and an offline PC would fail every attempt.
