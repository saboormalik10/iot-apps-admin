...
export class AnalyticsService {
  ...
  async metPressureTendency(orgId: string, deviceId: string, hours = 3) {
    if (!deviceId) throw new BadRequestException('deviceId is required');
    const toMs = Date.now();
   ...
   */
  async orgDeviceComparison(orgId: string, deviceIds: string[], sensor: string, from?: string, to?: string, interval = '1h') {
  ...
  async orgFleetHealth(orgId: string) {
    ...
    const metBytesPerDoc = await bytesPerDoc('metmeasures');
    ...
    const data = await Promise.all(
      devices.map(async (d) => {
        ...
        const totalReadings = recs.reduce((a, r) => a + (r.measureCount ?? 0), 0);
        const storageBytes = totalReadings * metBytesPerDoc;
        return {
