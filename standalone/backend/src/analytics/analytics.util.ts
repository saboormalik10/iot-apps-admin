 ...
 * descriptive statistics, interval bucketing, and unit conversion.
 */
...
const WIND_TO_MS: Record<string, (v: number) => number> = {
  ...
  knots: (v) => v * 0.514444,
  mph: (v) => v * 0.44704,
  // Beaufort force → m/s: v = 0.836 · B^1.5 (the WMO empirical relation).
...
const MS_TO_WIND: Record<string, (v: number) => number> = {
  ...
  knots: (v) => v / 0.514444,
  mph: (v) => v / 0.44704,
  // m/s → Beaufort force: the inverse, B = (v / 0.836)^(2/3). These two were
  // swapped, so 10 m/s converted to force 26.4 — a scale that stops at 12.
