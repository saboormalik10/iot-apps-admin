...
export class RolesController {
  ...
  private actor(user: JWTPayload, ipAddress: string | null = null): RoleActor {
    return {
      userId: user.userId,
      email: user.email ?? '',
      ipAddress,
  ...
  async create(@Body() body: RoleInputDto, @CurrentUser() user: JWTPayload, @ClientIp() ipAddress?: string | null) {
    return { data: await this.rolesService.create(body, this.actor(user, ipAddress)) };
  }
  ...
  @Patch(':id')
  @RequirePermissions('role:write')
  async update(@Param('id') id: string, @Body() body: RoleUpdateDto, @CurrentUser() user: JWTPayload, @ClientIp() ipAddress?: string | null) {
    return { data: await this.rolesService.update(id, body, this.actor(user, ipAddress)) };
  }
  ...
  async remove(
    ...
    return { data: await this.rolesService.remove(id, this.actor(user, ipAddress), replacementRoleId) };
  }
}
