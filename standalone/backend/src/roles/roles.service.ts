...
export class RolesService {
  ...
  async create(input: RoleInput, actor: RoleActor) {
    ...
    // The built-in roles live with `organizationId: null`, so an org-owned role
    // named "Viewer" did not clash — and then WON the lookup (resolve-role sorts
    // org-owned first), silently replacing the built-in one for every later
    // assignment. A role named "Admin" granting almost nothing would lock an
    // organisation out of its own administration.
    if (SEEDED_ROLES.some((r) => r.key === key)) {
      throw badReq(`"${name}" is a built-in role. Choose another name.`, 'RESERVED_ROLE_NAME');
    }
