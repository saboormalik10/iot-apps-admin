...
describe('permission catalogue', () => {
  ...
  it('keeps a hidden permission in the catalogue, so it is still enforced', () => {
    ...
    // `content:write` — still hidden, and still held by Admin so a bad record can
    // be corrected. (`import:write` was the cloud example; it went with the
    // import module.)
    expect(SEEDED_ROLES.find((r) => r.key === 'admin')!.permissions).toContain('content:write');
  });
...
describe('permission catalogue by audience', () => {
  ...
  // `device:write` is the platform-only example now; `station:provision` went with
  // the provisioning module. The mechanism under test is the same.
  it('hides platform-only permissions from a customer', () => {
    expect(keys(false)).not.toContain('device:write');
  });

  it('still shows them to a platform administrator', () => {
    expect(keys(true)).toContain('device:write');
  });
