...
describe('localHourStart / dayStartAt — the rain day (Phase 4)', () => {
  const { localHourStart, dayStartAt } = jest.requireActual('../src/utils/tz.util');
  const iso = (ms: number) => new Date(ms).toISOString();

  it('puts 9am local at 9am on a daylight-saving changeover day', () => {
    // Sydney moves from +10 to +11 at 02:00 on 4 Oct 2026. Local midnight + 9 hours
    // would land on 10:00 there; 9am is 22:00Z the day before.
    expect(iso(localHourStart('2026-10-04', 9, 'Australia/Sydney'))).toBe('2026-10-03T22:00:00.000Z');
    expect(iso(localHourStart('2026-10-03', 9, 'Australia/Sydney'))).toBe('2026-10-02T23:00:00.000Z');
  });

  it('before 9am, the 9am rain day is the one that began yesterday', () => {
    // 07:00 local on 22 Sep.
    const at = Date.parse('2026-09-21T21:00:00Z');
    expect(dayStartAt(at, 9, 'Australia/Sydney')).toEqual({ startMs: Date.parse('2026-09-20T23:00:00Z'), dayKey: '2026-09-21' });
    // A midnight day has already started.
    expect(dayStartAt(at, 0, 'Australia/Sydney')).toEqual({ startMs: Date.parse('2026-09-21T14:00:00Z'), dayKey: '2026-09-22' });
  });

  it('at exactly the start hour, the new day has begun', () => {
    const nineAm = Date.parse('2026-09-21T23:00:00Z');
    expect(dayStartAt(nineAm, 9, 'Australia/Sydney').dayKey).toBe('2026-09-22');
  });
});
