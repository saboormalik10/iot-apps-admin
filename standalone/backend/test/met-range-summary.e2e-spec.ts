...
describe('met/range-summary (e2e)', () => {
  ...
  describe('met/statistics', () => {
    ...
    it('returns the full profile the panel renders', async () => {
      // A YEAR, not a day: the profile only exists where there are readings, and
      // asking for "the last 24 hours" made this pass or fail on how recently the
      // database happened to be seeded rather than on the endpoint's shape.
      const res = await stats(365 * 86_400_000);
      expect(res.status).toBe(200);
      if (!res.body.count) {
        // An empty window is answered with the short shape on purpose (see the
        // "zero count rather than a stale figure" case above), and the panel
        // renders its empty state from `count` before reading any statistic.
        expect(res.body).toMatchObject({ count: 0, sensor: 'wind_speed' });
        return;
      }
    ...
    it('holds the invariants that must be true of any implementation', async () => {
      ...
      /**
       * Every statistic arrives rounded to 2 decimals, SEPARATELY. So these
       * invariants hold only within the rounding budget, and comparing at the
       * full 2-decimal precision fails on perfectly correct data:
       *   range vs max−min — three independent roundings, worst case 0.01 apart.
       *   variance vs stdDev² — squaring an already-rounded stdDev amplifies its
       *   error: a true stdDev of 2.1954 is stored as 2.20, whose square is 4.84
       *   against a variance of 4.82. The comparison is made in stdDev units
       *   instead, where both roundings together stay under 0.01.
       * This passed on production only because its calm wind (stdDev ≈ 0.23)
       * kept the amplified error small; the first real dataset with larger
       * values broke it.
       */
      expect(Math.abs(b.range - (b.max - b.min))).toBeLessThanOrEqual(0.011);
      ...
      expect(Math.abs(Math.sqrt(b.variance) - b.stdDev)).toBeLessThanOrEqual(0.01);
    });
  ...
  describe('fleet health', () => {
    it('returns the readings/day split and a storage estimate close to the real collection stats', async () => {
      ...
      const met = rows.find((r: { deviceId: string }) => r.deviceId === deviceId);
      ...
      // Compared with the collection's own on-disk figure, read now, rather than
      // a fixed literal. The literal (< 500 bytes) held on the cloud's millions
      // of well-compressed rows, but a small local database that other suites
      // have written to and deleted from sits nearer 800: WiredTiger keeps the
      // space. What matters is that the estimate follows the real collection —
      // a stale hardcoded constant regressing back in would not.
      if (met.totalRecords > 0) {
        const stats = (await mongoose.connection.db!.command({ collStats: 'metmeasures' })) as {
          count: number;
          storageSize: number;
        };
        const expectedMb = (met.totalRecords * (stats.storageSize / stats.count)) / (1024 * 1024);
        // Within 15%, or within the endpoint's own 0.01 MB rounding: straight
        // after seeding, before MongoDB's first checkpoint, the collection is a
        // few kilobytes on disk and any estimate of it rounds to zero.
        expect(Math.abs(met.storageEstimateMb - expectedMb)).toBeLessThanOrEqual(Math.max(0.011, expectedMb * 0.15));
      }
    });
