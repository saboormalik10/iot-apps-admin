...
describe('QC — step (time consistency)', () => {
  ...
  it('does not flag a 1 Hz sensor’s resolution flicker', () => {
    /**
     * The GMX551 sends temperature, humidity and pressure every second. A value
     * ticking between neighbours — 0.1 °C, 1 %RH — is the sensor's resolution,
     * not weather, but as a rate it is 6 °C/min. Found on the first live run:
     * 16 of 39 readings flagged.
     */
    const flicker = Array.from({ length: 60 }, (_, s) =>
      row(s * 1_000, { tempC: s % 2 ? 22.5 : 22.6, humidityPct: s % 2 ? 50 : 51, pressureHpa: s % 2 ? 1012 : 1012.1, dewPointC: s % 2 ? 11.6 : 12.0 }),
    );
    expect(applyQc(flicker).flaggedRows).toBe(0);
  });
