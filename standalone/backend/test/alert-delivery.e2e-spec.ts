...
import { conditionPhrase, evaluate, extremeWord, sensorLabel } from '../src/alert-rules/evaluate';
...
describe('what the alert actually says', () => {
  /**
   * The notification is read by a technician on a wall screen. It used to print
   * the rule's machine keys and an unexplained number:
   *
   *   "wind_speed gt 1m/s — read 18.29m/s"
   *
   * while the dashboard showed 7.26 m/s for the same minute — because a rule
   * fires on the minute's PEAK (deliberately, so a gust cannot slip between two
   * averages) and the dashboard shows its average. Same product, two numbers,
   * nothing to explain the difference. QA, 25 Sep 2026.
   */
  const body = (sensor: string, condition: string, threshold: number, unit: string, value: number) =>
    `${sensorLabel(sensor)} ${conditionPhrase(condition)} ${threshold}${unit}` +
    ` — ${extremeWord(condition)} ${value}${unit} in that minute`;

  it('names the sensor and the comparison in words', () => {
    expect(body('wind_speed', 'gt', 1, 'm/s', 18.29)).toBe(
      'Wind speed above 1m/s — peaked at 18.29m/s in that minute',
    );
    expect(body('dew_point', 'lte', 2, '°C', 1.4)).toBe(
      'Dew point at or below 2°C — dropped to 1.4°C in that minute',
    );
  });

  it('never leaks a raw sensor key or comparator', () => {
    for (const sensor of ['wind_speed', 'wind_dir', 'temperature', 'humidity', 'pressure', 'dew_point']) {
      for (const condition of ['gt', 'gte', 'lt', 'lte']) {
        const text = body(sensor, condition, 5, 'm/s', 9);
        expect(text).not.toMatch(/_/);
        expect(text).not.toMatch(/\b(gt|gte|lt|lte)\b/);
      }
    }
  });

  it('says which reading of the minute crossed the line', () => {
    expect(extremeWord('gt')).toBe('peaked at');
    expect(extremeWord('gte')).toBe('peaked at');
    expect(extremeWord('lt')).toBe('dropped to');
    expect(extremeWord('lte')).toBe('dropped to');
  });

  it('falls back to something readable for a sensor it does not know', () => {
    expect(sensorLabel('solar_radiation')).toBe('solar radiation');
  });
});
