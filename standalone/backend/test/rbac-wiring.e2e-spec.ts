...
describe('RBAC wiring (e2e)', () => {
  ...
  describe('write routes are authorised, not merely authenticated', () => {
    ...
    it('still lets a Viewer read a record and its readings', async () => {
      // Gated on `data:read`, which every seeded role holds. Adding a guard to a
      // READ route is the change most likely to take something away by accident,
      // so the reads are asserted rather than assumed.
      //
      // The cloud version also read the record's PICTURES. Those were photos the
      // mobile app uploaded to Cloudinary; the standalone has neither, so the
      // route is gone and the measures are what a viewer actually reads.
      ...
      if (first) {
        const measures = await request(http)
          .get(`/v1/records/${first._id ?? first.id}/measures`)
          .set('Authorization', `Bearer ${viewerToken}`);
  ...
  describe('roleId assignment', () => {
    ...
    /**
     * STANDALONE: the organisation admin holds EVERY permission in this
     * catalogue — provisioning, share links and import went with their modules —
     * so an admin can never escalate, and the cloud version of this test (which
     * leaned on `station:provision`) has nothing left to ask for.
     *
     * The escalation that is still real is a RESTRICTED ADMIN: a custom role on
     * the admin base — creating people needs both `user:write` and base role
     * `admin` — with the audit log taken away, handing out a role that has it.
     */
    const makeLimitedGranter = async () => {
      const limited = await Role.create({
        organizationId: new mongoose.Types.ObjectId(orgId),
        key: `m25-limited-${Date.now()}`,
        name: 'M25 Restricted admin',
        baseRole: 'admin',
        permissions: ['data:read', 'user:read', 'user:write', 'role:read'],
        isSystem: false,
      });
      createdRoles.push(limited._id as mongoose.Types.ObjectId);

      const email = `m25-limited-${Date.now()}@testing.invalid`;
      const made = await request(http)
        .post('/v1/organizations/me/users')
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ email, password: 'Str0ngPassphrase', roleId: String(limited._id) });
      expect(made.status).toBe(201);
      // An administrator chose that password, so the account can do nothing until
      // its owner replaces it (JwtAuthGuard). Do what a real user does first.
      const firstToken = await login(email, 'Str0ngPassphrase');
      const chosen = 'Their-Own-Passphrase-1';
      const changed = await request(http)
        .patch('/v1/users/me')
        .set('Authorization', `Bearer ${firstToken}`)
        .send({ currentPassword: 'Str0ngPassphrase', newPassword: chosen });
      expect(changed.status).toBe(200);
      return login(email, chosen);
    };

    it('refuses to grant a permission the granter does not hold', async () => {
      const token = await makeLimitedGranter();
      const auditor = await Role.create({
        organizationId: new mongoose.Types.ObjectId(orgId),
        key: `m25-auditor-${Date.now()}`,
        name: 'M25 Auditor',
        baseRole: 'viewer',
        permissions: ['data:read', 'audit:read'], // `audit:read` — the one withheld
        isSystem: false,
      });
      createdRoles.push(auditor._id as mongoose.Types.ObjectId);

      const res = await request(http)
        .post('/v1/organizations/me/users')
        .set('Authorization', `Bearer ${token}`)
        .send({ email: `m25-esc-${Date.now()}@testing.invalid`, password: 'Str0ngPassphrase', roleId: String(auditor._id) });
      expect(res.status).toBe(403);
      expect(res.body.error?.code ?? res.body.code).toBe('INSUFFICIENT_GRANT');
    });

    it('still lets that granter hand out a role within their own permissions', async () => {
      // Without this, the refusal above could be a blanket "limited users may not
      // add people" — which would pass the test while being the wrong rule.
      const token = await makeLimitedGranter();
      const reader = await Role.create({
        organizationId: new mongoose.Types.ObjectId(orgId),
        key: `m25-reader-${Date.now()}`,
        name: 'M25 Reader',
        baseRole: 'viewer',
        permissions: ['data:read'],
        isSystem: false,
      });
      createdRoles.push(reader._id as mongoose.Types.ObjectId);

      const res = await request(http)
        .post('/v1/organizations/me/users')
        .set('Authorization', `Bearer ${token}`)
        .send({ email: `m25-ok-${Date.now()}@testing.invalid`, password: 'Str0ngPassphrase', roleId: String(reader._id) });
      expect(res.status).toBe(201);
    });
  });
