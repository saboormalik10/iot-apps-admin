...
export function DeviceStatusTable() {
  ...
  // Only when something actually reports one — a mains-powered station never does,
  // and a column of empty grey meters looks like a fault rather than "no battery".
  const anyBattery = (devices ?? []).some((d) => d.lastBatteryPct !== null && d.lastBatteryPct !== undefined);

  return (
    <Card className="p-4">
      <h2 className="mb-3 text-sm font-medium">Station status</h2>
      {isLoading ? (
        <TableSkeleton rows={4} cols={3} />
      ) : !devices || devices.length === 0 ? (
        <EmptyState title="No station yet" body="The station is created when the software is installed. If this is empty, the setup step did not finish — see the installation guide." />
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b text-left text-muted-foreground">
                ...
                {anyBattery ? <th className="py-2 pl-2 font-medium">Battery</th> : null}
              </tr>
            </thead>
            <tbody>
              {devices.map((d) => (
