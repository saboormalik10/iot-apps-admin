 ...
 * would blank them.
 */
export function useDashboardRealtime(deviceIds: { met?: string }) {
  ...
  useSocketEvent<MetLatestPayload>(ClientEvent.MET_LATEST, (payload) => {
    if (deviceIds.met) {
      ...
      // Rain today / last hour / rate: a difference of totals the server works
      // out, so each new minute asks again. Cheap: four indexed lookups.
      qc.invalidateQueries({ queryKey: queryKeys.metRain(deviceIds.met) });
    }
  });
  ...
  // Alerts arrive as notification:new → the bell (Month 7) already refetches the
  // feed; here we also refresh the summary tiles.
