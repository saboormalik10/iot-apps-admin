...
export function useScopedDevice(type: DeviceType): {
  ...
  const resolved = useMemo(() => {
    ...
    /**
     * `isAuto` means "we chose one FOR you", which the screens print as
     * "(auto-selected)". With a single station there was nothing to choose
     * between, and saying so reads as though something else was expected — this
     * product runs one station per PC, so that was every screen, always.
     */
    const isAuto = !explicit && ofType.length > 1 && Boolean(device);
    return { deviceId: device?._id, device, isAuto, candidates: ofType };
