...
import { useSiteTimeZone } from '@/lib/hooks/use-site-timezone';

// The STATION's time, not the viewer's PC's — the same reading was showing two
// different times on two screens five hours apart.
const fmt = (ms: number, tz: string) =>
  new Date(ms).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short', timeZone: tz });
...
export function RecordsList() {
  ...
  const columns = useMemo<ColumnDef<MetRecordRow, unknown>[]>(
    () => [
      ...
      { header: `Started (${tz})`, cell: ({ row }) => fmt(row.original.dateStartMs, tz) },
      {
        header: 'Duration',
        cell: ({ row }) => {
    ...
    // `tz` is read by the Started cell, and arrives a moment after first render.
    [window, tz],
  );
