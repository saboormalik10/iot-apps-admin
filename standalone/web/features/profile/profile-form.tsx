...
export function ProfileForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Changing the password ends every session the account had — this one too,
      // which used to sign the user out within 15 minutes, mid-work. Sign straight
      // back in with the new password to carry on.
      if (changingPassword) await authApi.login(profile!.email, newPassword);
      ...
      if (err instanceof ApiError && err.code === 'CURRENT_PASSWORD_INCORRECT') {
        setErrors({ currentPassword: 'auth.currentPasswordWrong' });
      } else if (err instanceof ApiError && err.code === 'SAME_PASSWORD') {
        setErrors({ newPassword: 'auth.samePassword' });
      } else {
        apiToast.error(err);
      }
    }
