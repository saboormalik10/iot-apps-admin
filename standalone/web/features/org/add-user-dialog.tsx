...
export function AddUserDialog() {
  ...
  const options = roles;
  /**
   * What a new person gets when nobody chooses: the site's DEFAULT role.
   *
   * This used to be `options[0]`, and the list is sorted least-privileged first
   * — so any custom role narrower than Viewer became the silent default for
   * every account created afterwards. A one-permission role made for a
   * contractor would quietly become what everyone starts with. (QA, 24 Sep 2026,
   * found new accounts being handed a leftover role called "NaN".)
   */
  const fallbackRole =
    options.find((r) => r.isDefault) ??
    options.find((r) => r.isSystem && r.baseRole === 'viewer') ??
    options.find((r) => r.isSystem) ??
    options[0];
  const selected = roleId || fallbackRole?._id || '';
