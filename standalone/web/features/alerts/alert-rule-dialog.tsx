...
export function AlertRuleDialog({
  ...
  const appType: AlertAppType = 'MET';
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Fires a notification when a station&apos;s reading crosses your threshold. Repeat alerts are
        ...
        <div className="space-y-4">
          <div className="space-y-1">
            ...
            <Input id="rule-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Strong wind" aria-invalid={Boolean(errors.name)} />
            {errors.name ? <p className="text-xs text-status-error">{errors.name}</p> : null}
          </div>

          {/* Device(s) */}
          <div className="space-y-1.5">
            <Label>{isEdit ? 'Station' : 'Station(s)'}</Label>
            {isEdit ? (
              ...
              <p className="rounded-md border border-dashed px-3 py-2 text-sm text-muted-foreground">
                No station at this site reports {APP_TYPE_TO_DEVICE_TYPE[appType]} readings yet.
              </p>
            ) : (
              <>
                ...
                <p className="text-xs text-muted-foreground">
                  {deviceIds.length > 1
                    ? `This rule will be created on ${deviceIds.length} stations.`
                    : 'Choose the station this rule watches.'}
                </p>
              </>
            )}
          ...
          <div className="space-y-1.5">
            ...
            {users.length === 0 ? (
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground">
                  {notifyUserIds.length === 0 ? 'No one selected → everyone at the site is notified.' : `${notifyUserIds.length} selected.`}
                </p>
              </>
            )}
          </div>
