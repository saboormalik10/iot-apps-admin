...
export function TimeZonePicker({
  ...
  return (
    <Select
      ...
      // Never pass on an EMPTY value. The options are built only while the list is
      // open, so Radix's hidden native <select> has no option for the current zone;
      // when the form first loads its value, Radix fires a change and reads back ""
      // - which blanked the form's zone ("Select a timezone" on a site that has one)
      // and blocked saving until one was picked again. Nobody can choose "" here.
      onValueChange={(tz) => {
        if (tz) onChange(tz);
      }}
    >
