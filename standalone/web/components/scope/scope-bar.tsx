...
const HIDDEN_PREFIXES = [
  '/org',
  // The query screen picks its own period, in the station's time.
  '/query',
  // The PC's health: nothing scoped.
  '/system',
  '/settings',
  '/profile',
  '/import',
  ...
  '/share',
  '/users',
  // The audit log carries its own period buttons (Today / 7 days / 30 days / All
  // time). A second range control above them, set to something else, is two
  // filters contradicting each other.
  '/audit',
];
...
export function ScopeBar() {
  ...
  /**
   * With one station — this product's normal case — a station picker and a type
   * picker can only ever choose the thing already on screen. A control that
   * cannot change anything reads as a broken control, so both step aside and the
   * bar is left with the one filter that does something: the period.
   */
  const showPickers = devices.length > 1 || availableTypes.length > 1;
  if (!showPickers && !showRange) return null;

  return (
    <div className="flex flex-wrap items-center gap-2 border-b bg-card/40 px-4 py-2 text-sm md:px-6">
      <span className="mr-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">
        {showPickers ? 'Scope' : 'Period'}
      </span>
