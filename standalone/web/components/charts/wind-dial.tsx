...
export function WindDial({
  ...
  /**
   * What an UNcalibrated bearing is measured from: the GMX551's compass
   * (`magnetic`) or its own north marker (`mast`, and the default). Ignored once a
   * heading offset is set — the bearing is then true.
   */
  ...
  const caption = calibrated
    ? null
    : reference === 'magnetic'
      ? { short: 'Magnetic north · declination not set', long: ', bearing from magnetic north, declination not set' }
      : { short: 'Relative to mast · uncalibrated', long: ', bearing relative to the mast, uncalibrated' };
    ...
    <div className={cn('flex flex-col items-center gap-2', className)}>
      ...
      <div
        ...
        {...(hasSpeed
          ...
          : { role: 'img' })}
      >
        {/* Scales down where the column is narrower than the dial, like every
            other instrument here (`Gauge` does the same). A fixed 220px width
            made the dashboard card wider than a 375px screen, so the whole page
            scrolled sideways (QA, 25 Sep 2026). */}
        <svg
          width="100%"
      ...
      <div className="-mt-2 flex flex-col items-center gap-1 text-center">
        ...
        {caption && bearing != null ? (
          <span className="text-[10px] uppercase tracking-wide text-muted-foreground">{caption.short}</span>
        ) : null}
      </div>
    </div>
  );
