...
describe('TimeZonePicker search', () => {
  ...
  // Found on the site build: the org form starts empty and fills in the stored
  // zone a moment later. Radix then fired a change carrying "" (its hidden native
  // <select> has no options until the list opens), and the form blanked itself.
  it('keeps a zone loaded after mount, and never reports an empty change', async () => {
    const onChange = vi.fn();
    function Form() {
      const [tz, setTz] = React.useState('');
      React.useEffect(() => setTz('Australia/Melbourne'), []);
      // Inside a <form>, as on the org page: only then does Radix add its hidden <select>.
      return (
        <form>
          <TimeZonePicker value={tz} onChange={(v) => { onChange(v); setTz(v); }} />
        </form>
      );
    }
