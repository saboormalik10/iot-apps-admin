...
export async function listDevices(opts: ListDevicesOptions): Promise<ListDevicesResult> {
  ...
  // Compute isOnline on the fly (field in DB is stale after restart)
...
export async function getDevice(organizationId: string, deviceId: string): Promise<IDevice> {
...
export async function updateDevice(
  ...
  const device = await getDevice(organizationId, deviceId);

  const before = { name: device.name, customName: device.customName, serialNo: device.serialNo, firmwareVersion: device.firmwareVersion };
...
export async function getDeviceStats(organizationId: string, deviceId: string) {
  await getDevice(organizationId, deviceId); // 404 guard
