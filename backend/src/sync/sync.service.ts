...
export async function getSyncStatus(organizationId: string, deviceId?: string) {
...
export async function syncUpload(organizationId: string, payload: SyncUploadPayload) {
  if (!payload.type) {
    const err = new Error('type is required: "nep_session" or "met_record"');
    (err as NodeJS.ErrnoException).code = 'VALIDATION_ERROR';
    throw err;
  }

  if (payload.type === 'nep_session') {
    return await _upsertNepSession(organizationId, payload);
  }
  return await _upsertMetRecord(organizationId, payload);
}

async function _upsertNepSession(organizationId: string, payload: SyncUploadPayload) {
  if (!payload.sessionId || !payload.deviceId || !payload.startTimestamp) {
    const err = new Error('sessionId, deviceId and startTimestamp are required for nep_session');
    (err as NodeJS.ErrnoException).code = 'VALIDATION_ERROR';
    throw err;
  }
  ...
  // Idempotent upsert by sessionId UUID
...
async function _upsertMetRecord(organizationId: string, payload: SyncUploadPayload) {
  if (!payload.deviceId || !payload.dateStart) {
    const err = new Error('deviceId and dateStart are required for met_record');
    (err as NodeJS.ErrnoException).code = 'VALIDATION_ERROR';
    throw err;
  }
  ...
  const { parseMeasureSentence, isHeaderSentence, parseTimestampMs } = await import('../utils/measure-parser.util');
  ...
  // Idempotent upsert by localRecordId if provided, else by dateStart+deviceId
