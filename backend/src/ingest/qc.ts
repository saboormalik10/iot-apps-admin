...
import type { ParsedMetRow } from './met-csv/parse-met-csv';
...
const LIMITS: Readonly<Record<QcField, FieldLimits>> = Object.freeze({
  ...
  tempC: { min: -80, max: 60, maxRatePerMin: 5, persistMinutes: 60 },
  ...
  humidityPct: { min: 0, max: 100, maxRatePerMin: 30, persistMinutes: 60, persistExempt: 100 },
  ...
  pressureHpa: { min: 500, max: 1100, maxRatePerMin: 10, persistMinutes: 120 },
  dewPointC: { min: -80, max: 60, maxRatePerMin: 5, persistMinutes: 60 },
  ...
  // Cumulative over a day; no rate or persistence check — no rain is the norm.
  precipMm: { min: 0, max: 2000, maxRatePerMin: null, persistMinutes: null },
});
 ...
 */
const STATUS_OK = Object.freeze(new Set(['A', '0', '00', 'OK']));
...
export function applyQc(rows: readonly ParsedMetRow[], state: QcState = {}): QcResult {
  ...
  const out = ordered.map((row) => {
    ...
    if (next.status !== null && !STATUS_OK.has(next.status.trim().toUpperCase())) {
    ...
    for (const field of FIELDS) {
      ...
      const gapMs = prev ? next.timestampMs - prev.lastTsMs : Infinity;
      ...
      if (continuous && limits.maxRatePerMin !== null) {
        const ratePerMin = (Math.abs(value - prev!.lastValue) / gapMs) * 60_000;
        if (ratePerMin > limits.maxRatePerMin) {
          flag(`step:${field}`);
          next[field] = null;
          rejected = true;
        }
